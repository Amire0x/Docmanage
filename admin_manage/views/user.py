...
def index(request,pIndex=1):
    ...
    return render(request,"admin_manage/user/index.html",context)

def add(request):
    '''加载信息添加表单'''
    return render(request,"admin_manage/user/add.html")
...
def insert(request):
    ...
    try:
        ...
        print(err)
        context = {'info':"添加失败！"}
    return render(request,"admin_manage/info.html",context)
...
def edit(request,uid=0):
    ...
    try:
        ob = User.objects.get(id=uid)
        context = {'user':ob}
        return render(request,"admin_manage/user/edit.html",context)
    except Exception as err:
        print(err)
...
def update(request,uid):
    ...
    try:
        ob = User.objects.get(id=uid)
        ob.nickname = request.POST['nickname']
        ob.department = request.POST['department']
